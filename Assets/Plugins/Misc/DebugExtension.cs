...
#region DebugDrawFunctions
    ...
    /// </param>
    public static void DebugLocalCube(Transform transform, Vector3 size, Color color, Vector3 center = default,
    ...
    ///     - Whether or not the cube should be faded when behind other objects.
    /// </param>
    public static void DebugLocalCube(Transform transform, Vector3 size, Vector3 center = default,
        float duration = 0, bool depthTest = true)
    {
    ...
    ///     - Whether or not the cube should be faded when behind other objects.
    /// </param>
    public static void DebugLocalCube(Matrix4x4 space, Vector3 size, Color color, Vector3 center = default,
        float duration = 0, bool depthTest = true)
    {
        color = color == default ? DefaultColor : color;
    ...
    ///     - Whether or not the cube should be faded when behind other objects.
    /// </param>
    public static void DebugLocalCube(Matrix4x4 space, Vector3 size, Vector3 center = default,
        float duration = 0, bool depthTest = true)
    {
        ...
        bool depthTest = true)
    {
        ...
        var x = new Vector3(position.x, position.y + radius * Mathf.Sin(0),
            position.z                             + radius * Mathf.Cos(0));
        var y = new Vector3(position.x + radius * Mathf.Cos(0), position.y,
            position.z                 + radius * Mathf.Sin(0));
        var z = new Vector3(position.x + radius * Mathf.Cos(0),
...
#region GizmoDrawFunctions
    ...
    {
        DrawBounds(bounds, DefaultColor);
    }
    ...
    ///     - The color of the cube.
    /// </param>
    public static void DrawPath(Vector3[] path, Color color, float pointSize = 1f)
    {
        Color oldColor = Gizmos.color;
        Gizmos.color = color;
        ...
        do
        {
            Vector3 currentPos = path[index];
            DrawPoint(currentPos, color, pointSize);
            if (index > 0) Gizmos.DrawLine(currentPos, prePos);
            prePos = currentPos;
            index++;
    ...
    public static void DrawBezierPath(BezierGizmo[] path, Color color, float pointSize = 1f)
    {
        Color oldColor = Gizmos.color;
        Gizmos.color = color;

        var index = 0;
        BezierGizmo prev = null;
        do
        {
            BezierGizmo current = path[index];
            DrawPoint(current.point, color, pointSize);
            if (index > 0)
            {
                if (prev                      == null) continue;
                if (prev.numberOfSubdivisions <= 0) prev.numberOfSubdivisions = 1;
                Vector3[] array = new Vector3[prev.numberOfSubdivisions + 1];
                for (var i = 0; i <= prev.numberOfSubdivisions; i++)
                {
                    var t = i / (float) prev.numberOfSubdivisions;
                    var omt = 1.0f - t; // One minus t = omt
                    array[i] = prev.point      * (omt * omt     * omt) +
                               prev.tangent    * (3 * omt * omt * t)   +
                               current.point   * (3 * omt * t   * t)   +
                               current.tangent * (t * t         * t);
                    if (i > 0) Gizmos.DrawLine(array[i - 1], array[i]);
                }
            }
    ...
    {
        DrawLocalCube(transform, size, DefaultColor, center);
    }
    ...
    {
        DrawLocalCube(space, size, DefaultColor, center);
    }
    ...
    public static void DrawCircle(Vector3 position, Vector3 up, Color color, float radius = 1.0f)
    {
        ...
        Color oldColor = Gizmos.color;
        Gizmos.color = color == default ? DefaultColor : color;
    ...
    {
        DrawCircle(position, position, DefaultColor, radius);
    }
    ...
    {
        DrawCircle(position, Vector3.up, DefaultColor, radius);
    }
    ...
    {
        DrawCylinder(start, end, DefaultColor, radius);
    }
    ...
    {
        DrawCone(position, direction, DefaultColor, angle);
    }
    ...
    {
        DrawCone(position, Vector3.up, DefaultColor, angle);
    }
    ...
    {
        DrawCapsule(start, end, DefaultColor, radius);
    }
